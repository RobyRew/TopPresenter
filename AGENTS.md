...
- **Sidebar is ONE native List (v10.11)**: content modules + utility destinations (History · Settings · Account) are two Sections of the same `List(selection:)` `.listStyle(.sidebar)` — no custom row chrome; selection follows `.tint`. The 10-click advanced unlock rides a `simultaneousGesture(TapGesture)` on the Settings row so native selection still works.
