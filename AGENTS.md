...
- **IMPORT IS ONE PATH (v1)**. Every entry point — the menu items, each library's Import button, a window drop, a Finder double-click — posts `.importFiles` and lands in `UniversalImportSheet`, hosted ONCE in MainControlView. There were five interfaces with different capabilities; do not add a sixth. The pieces, in order:
  - `ImportCatalog` — the ONE answer to "what can we import?", DERIVED from `SupportedBibleFormat`/`SupportedSongFormat`/`MediaKind`. Adding a format here reaches the open panel, the folder walk, the classifier and the in-app format list at once. Never hand-roll an extension list again (there were four, and they disagreed: `.flv` was a video to one and an image to another)
  - `ImportScanner` — expands a selection. Budget, NOT a depth cap: `maxDepth 8`, `maxCandidates 5000`, `maxVisitedEntries 200k`, and every truncation is REPORTED (`Result.truncations`). The old depth-3 cap silently returned half a folder. The beach-ball protection is the extension filter — it never opens a file it cannot use — not refusing to look deep. Extensionless files are probed (OpenSong) via mapped prefix reads. Top-level entries that yielded nothing come back in `Result.rejected`; files found by walking INTO a folder do not (listing a Documents tree one file at a time is noise)
  - `DuplicateResolver` — a pure function over value types, matching by INDEX not `PersistentIdentifier`, so the batch and interactive paths cannot drift apart. Identity ladder per kind (first hit wins, each with a confidence), then a content comparison to grade it. Identical content is ALWAYS skipped, whatever the policy: re-dropping a file you already have is not a conflict. `matchedOn` and `differences` are written for the dialog — an operator can only choose well if they can see what was compared
  - `ImportCoordinator` — runs kinds in `kindOrder`: **media, bible, song, theme, slides, session**. Media BEFORE sessions is a correctness rule, not a preference: `importSession` re-links media against the library as it stands, so a session imported first arrives with everything missing. Its `Summary` distinguishes imported/skipped/replaced/merged/failed/unsupported — a bare count cannot tell "12 imported" from "2 imported, 10 already there", which is how a silent double-import goes unnoticed
- **Native formats own their extensions (v1)**: `.tpbible` `.tpsong` `.tpsongcollection` `.tpslides` `.tpschedule` `.tptheme` — all in `TopPresenterFormat`, all declared in Info.plist with `UTTypeIconFile` + `LSHandlerRank: Owner`. They still ARE JSON (`UTTypeConformsTo: public.json`), so Quick Look, `jq` and git all still work; what the extension buys is the icon, the double-click and an open panel that offers OUR documents. **`.json` is no longer read or written** — a `.json` is refused with an actionable message (rename / re-export), never "unrecognized file". Filenames come from `ExportNaming`: `<Name>[_<Qualifier>]_TopPresenter_<Type>.<ext>`, redundant on purpose because Windows and WhatsApp hide the extension
- **Export is TopPresenter-only (v1)**: plain-text and CSV writers are GONE. They were lossy one-way doors — what they wrote could never be read back. Only TopPresenter reads what TopPresenter writes, and everything it writes it reads back whole
...
- **Bible navigation is by COORDINATE (v1.1)**: `Core/Persistence/BibleNavigator.swift` (app-global `.shared`, invalidated on any `.libraryDidChange` naming Bibles) resolves `(module, book, chapter)` with a targeted `FetchDescriptor` + `relationshipKeyPathsForPrefetching = [\.verses]`, caches resolved passages by coordinate (cap 24) and caches the SPINE separately — `bookNumbers`, `chapterNumbers`, `chapterCounts` — because crossing a boundary needs only integers. NEVER walk `module.books` / `book.chapters` / `chapter.verses` to FIND something: `stepBibleAnchor` used to fetch every module, fault all 66 books, sort every chapter and then sort its verses ON EVERY ←/→ PRESS, and `book.chapters.count` in the book list faulted every chapter of every book per render (×3 inside `ViewThatFits`). `adjacentChapter` crosses into the neighbouring book and walks past books with no chapters; `prefetchNeighbours` warms both sides after a present. `LibraryManager.canAdvanceToNextChapter`/`canReturnToPreviousChapter` are STORED and recomputed in `refreshCachedVerses` — the preview panel reads them four times per render, so as computed properties they re-faulted the module spine every frame.
- **MEASURED on a real library (v1.1) — `TopPresenterTests/RealLibraryPerformanceTests.swift`**: the only performance evidence that counts. It opens a COPY of a real store (`TEST_RUNNER_TP_REAL_STORE=…/default.store`, always under the app container's `Data/tmp/` — the sandboxed test host cannot create Core Data's `_EXTERNAL_DATA` beside a store in `/private/tmp`), times the paths a service exercises with the OLD code re-implemented inline against the NEW, and writes a markdown table (`TEST_RUNNER_TP_PERF_OUT`). `RealLibraryScaleUp` imports `.tpsong` files from `TEST_RUNNER_TP_SCALE_SOURCE` until `TEST_RUNNER_TP_SCALE_TARGET` songs (Sunday's 40k came from `ExtraAssets/`). Skipped when unset, so CI never sees it. Every number below is from it, at 40 000 songs / 820 733 verses unless stated. **Do not "optimise" this codebase without running it first** — three of the changes in this section were guesses that measured WORSE before they were fixed.
- **`relationshipKeyPathsForPrefetching` makes SwiftData aggregate loops SLOWER (v1.1)**: `v.song?.id` over 39k verses took 19 s faulting and 25 s prefetched; `ver.song?.id` over 43k versions 24 s → 34 s. SwiftData materialises the full related object per row. Prefetch is right for ONE parent and its children (`BibleNavigator.passage` prefetches `verses` for one chapter); it is wrong for "every row of a table and its parent". Never add it to a loop without the harness.
- **The song index is built from COLUMNS, not objects (v1.1)**: `SearchIndexBuilder.buildSongsFromColumns` runs dictionary-result `NSFetchRequest`s (`.dictionaryResultType`, relationships projected as `NSManagedObjectID`s, version counts via `propertiesToGroupBy: ["song"]` + a `count:` expression) on a `NSManagedObjectContext` over `SwiftDataModelBridge.coordinator(for:)` — official Core Data, nothing instantiated, nothing faults. **73 s → 8.2 s** for the full build, and the harness asserts the result is field-for-field IDENTICAL to the SwiftData walk (`buildSongsByWalking`, kept as the fallback for in-memory stores). `Song.webURL(inExtensionsJSON:)` is the static form the column path shares with the model.
- **A song edit updates ONE index entry (v1.1)**: post `Notification.Name.postSongChange([ids])` from anything that knows which songs it touched (editor save, verified toggle, move, create). `SearchIndex.scheduleRebuild(changed:songIDs:)` keeps the ids; when every song post in the window named its songs and there are ≤ 64, `applyIncrementally` re-projects those via `builder.buildSong(id:)` and swaps them in — `TokenIndex.replacing(entry:oldBlob:newBlob:)` edits only the postings that changed. An un-named `.song` post (import batch, bulk delete) or a DELETE (positional postings cannot lose an entry) falls back to the full build. Both paths share `SearchIndexBuilder.project(_:firstLine:versionCount:)`, and `TokenIndexIncrementalTests` asserts the incremental result equals a full rebuild. Before this, one verified-toggle re-walked the library and contended the store with the next Bible step — the "15 s per verse" on a Sunday.
- **`TokenIndex.build` walks unicode SCALARS and runs chunked across cores (v1.1)**: 13.4 s → 3.1 s over 27 MB of lyrics. `TokenIndex.tokens(in:)` is the ONE tokeniser (build, incremental swap, and `searchTokens` all use its word rule); a `Character` walk on already-folded text paid grapheme segmentation for nothing. Chunks are contiguous ranges so concatenated postings stay ascending, which `match` relies on.
- **Ranking: precomputed, and sort LIGHT keys (v1.1)**: `SearchIndex.rankTable(for:)` packs `priority.rank` for every song once per (generation, rules) — rank is a function of the entry and the rules, never the query — and `PaletteSnapshot.ranks` carries it to the detached palette query. Inside `PaletteSearch.songBuckets` the thing sorted is `(index, rank, popularity, foldedTitle)`, NOT the entry: sorting 17-field entries with ten refcounted Strings retained/released ten Strings per swap, and that — not folding, not ranking, not bridged strings (all three were tried and measured flat) — was most of the 600 ms a 22 000-hit query cost. **3.9 s → 178 ms.** `SongIndexEntry.foldedTitle` is folded once at build time.
- **Chapter counts are the relationship's own COUNT(*) (v1.1)**: `BibleNavigator.chapterCounts` reads `book.chapters.count` per book, once per module, cached. It was rewritten twice on theory before being measured: prefetching every chapter object cost 89 ms cold, a `fetchCount` with a `$0.book?.id == bookID` predicate cost 252 ms (it joins through the UUID column, not the FK) — the original faults cost 22 ms and the cached form costs 16 ms.
- **Update channel is PERSISTED and the feed is immutable per release (v1.1)**: `UpdateChannel` (stable|beta) lives in UserDefaults (`updateChannel`) and seeds the Sparkle delegate; the old beta toggle was a plain `var` on the delegate and forgot itself on every launch. Stable items carry no `<sparkle:channel>`, betas carry `beta`. Every release has its OWN asset URL (`releases/download/v<full-version>/TopPresenter-<full-version>.zip`); the rolling `v<ver>-alpha` tag that overwrote one URL per push left 39 of 40 feed items pointing at bytes that no longer existed ("improperly signed"), all on the beta channel, so a fresh install on the stable default saw NOTHING. `appcast_upsert.py` drops every `-alpha` item (all verified 404), caps items PER CHANNEL (`MAX_ITEMS_PER_CHANNEL`, 10) and writes `sparkle:minimumSystemVersion` from `MIN_OS`.
- **The updater on the developer's Mac fails because the app is AD-HOC SIGNED (v1.1, measured)**: with the installed 0.1.0, every Sparkle check hangs 60 s and fails with `NSURLErrorDomain -1001 "The request timed out"` on the feed URL, while Little Snitch's network extension logs `Could not find app info … No current verdict available` at the same millisecond — it cannot identify a process with `Signature=adhoc, TeamIdentifier=not set`, so it never issues a verdict and the flow is held until the app gives up. The feed, the channel logic and Sparkle were all fine (`curl` fetches it instantly; the EdDSA signature verifies). This reproduces on ANY Mac with a Network-Extension firewall. Fixes: a Developer ID signature (CI does it when the six signing secrets exist — see the workflow) or, per machine, an explicit Little Snitch allow rule for `/Applications/TopPresenter.app`. Diagnosis tools: `/usr/bin/log show --predicate 'process == "TopPresenter" AND subsystem == "org.sparkle-project.Sparkle"'` and `… 'subsystem CONTAINS "obdev"'`. **`log` in this repo's Bash sandbox is zsh's builtin, not `/usr/bin/log` — always use the full path, or every query silently returns nothing.**
- **Release paths (v1.1)**: `main` push / PR = build + tests only. `beta` branch push = `v<MARKETING_VERSION>-beta.N` (N = highest existing + 1), pre-release with immutable assets, feed item on `beta`. Tag `v<MARKETING_VERSION>` (no `-`) = stable release + feed item with no channel; the tag must equal the project's `MARKETING_VERSION` or the build fails. The bundle's `CFBundleShortVersionString` is set to the FULL string (`1.0.1-beta.3`) at build time so the About box, the Versions list and the feed agree; `CFBundleVersion` is the run number, monotonic across every branch and tag, which is what Sparkle compares. The `github-pages` environment must allow the `beta` branch and `v*` tags (both rules are set).
- **Library sweeps never run in `body` (v1.1)**: a filter/sort/group pass over `SearchIndex.songs` belongs in `.task(id: <inputs>)` writing `@State`, never in a computed property `body` reads. `SongListPanel` had `filtered` (computed) read three times per render plus a grouping pass — four sweeps of 40k entries every time anything re-rendered the panel, including selecting a song. The input struct must name EVERY dependency (query, sort key, each filter, `index.generation`, `pinStore.pinnedSongIDs`, `priority.rules`) or the list goes stale. Same rule for `SongSlideFilmstrip.slides` (`buildSongSlides` decodes every section's `linesJSON`) and the per-version chord map (`SongChordIndex`). `SongSlide.id` is derived (`"<sectionKey>#<index>"`), NOT a fresh `UUID()` — a minted id makes `ForEach` treat every slide as new and rebuild every thumbnail.
- **Every library mutation NAMES its kind (v1.1)**: post via `Notification.Name.postLibraryChange(.song)` — never a bare `.libraryDidChange`. An un-named post means "assume everything", and `SearchIndex.rebuildNow` then re-walks the whole song library (three full-table fetches) because a photo was deleted or a session renamed.
- **Songs can be CREATED, not only imported (v1.1)**: `SongFactory.create(title:in:context:)` makes a song with its Original version, one empty verse section and the flattened `SongVerse`, so it is editable AND presentable immediately. New songs land in the collection marked `sourceFormat == SongFactory.manualSourceFormat` ("manual") — matched on the marker, never the localized name, so renaming it does not start a second one. The editor discards a never-kept song on Cancel via `LibraryManager.songEditIsNew`; reverting it to its empty snapshot would leave an untitled row behind.
- **Section kinds and labels (v1.1)**: `Core/SongSections.swift`. `SongSectionKind.classify` maps whatever an importer wrote (`chorus`/`refren`/`estribillo`/`Pre-Chorus`…) onto a canonical kind — longest-prefix first, so "prechorus" is never read as "chorus", and anything unrecognised stays `.other` (forcing it to `.verse` renumbers the projector). `SongSectionLabeling.labels(forTypes:)` names sections in the APP's language with their position: verses read „2/4" against the VERSE count, other kinds read their name, numbered only when the song has more than one. Governed by `song_sectionLabelsInAppLanguage` (default on); `songTypeColor`/`songTypeLabel`/`songSectionTypes` are shims over the enum — do not reintroduce parallel string switches.
- **Result priority is the operator's, and configurable (v1.1)**: `Core/Search/SongPriority.swift` — ordered `SongPriorityBand`s over a fixed set of `SongFacet`s (songbook/author/collection/source/website/language), each with an ordered value list; `restrictedToValues` is what separates "any song with an author" from "songs written here" (a value of a facet every song has). Persisted by `SongPriorityStore` (app-global, UserDefaults). Applied INSIDE the relevance buckets of `PaletteSearch.songBuckets` — relevance still wins, priority breaks ties, and priority outranks popularity because one is configured and the other inferred. Edited in Settings ▸ Cântece, whose value picker is fed from `SearchIndex.songs` so a book name cannot be typed in a form that silently matches nothing.
- **ONE song ranking (v1.1)**: ⌘K and the Songs tab both call `PaletteSearch.songBuckets`/`rankedSongList` — title-prefix, then title-contains, then lyrics/author, with the fuzzy fallback. The Songs tab used to AND-match and render alphabetically, so the same query ranked differently per box and one typo returned nothing. While a query is active the browser shows ONE flat section (A-Z headings would scatter the ranking) and `ScrollViewReader` reveals the selected song — ⌘K selected results the list never scrolled to.
...
### Testing Gotchas
- When inserting a declaration above an existing one, check the line ABOVE the anchor: `@discardableResult`, `@ViewBuilder` and friends sit on their own line, and anchoring on the `func` slides the new code between the attribute and its function. It compiles as a duplicate attribute on YOUR declaration, which reads as if your code is wrong. This has cost time three times
...
- **`makeTestManager()` is headless, and that is load-bearing.** `PresentationManager` finds its output window through `NSApplication.shared.windows` — process-global — so every test manager used to drive the test host's REAL presentation window. When that window exists and is hidden, `presentContent` stages its apply 60 ms late, and any test asserting right after a show reads stale content. Tests that hide it (`hidePresentationWindow`, `hideOutputNow`, an `ask` disconnect) therefore changed the behaviour of every test running in PARALLEL with them, and `liveContentCarriesVerseRuns` failed about one run in five with nothing in its own body to explain it. The window source is injected now: `makeTestManager()` passes `{ [] }`, and the two tests that are genuinely about staging use `makeStagingTestManager()`, which owns its window. Guarded by `atestManagerIsHeadlessSoEveryShowAppliesImmediately`
- **A flake like that will not reproduce under `-only-testing`.** Cross-test interference through process-global state only shows in a full parallel run, and only sometimes — so "passes in isolation" is evidence of a race, not of correctness. Confirm a fix with at least five consecutive full runs
...
### English is the base language — this is not a preference

`CFBundleDevelopmentRegion = en` and the catalog's `sourceLanguage` is `en`. **Every other language falls back to English**, and English falls back to nothing: a key with no `en` entry resolves to the raw key TEXT.

That matters because roughly 44% of the ~1400 `String(localized:)` **keys are written in Romanian**. A key is not a source language — it is an identifier that happens to be spelled in Romanian. So an empty `en` slot does not show "the Romanian version", it shows an identifier, to a German user, an English user, everyone.

> An earlier revision of this file claimed *"a Romanian key needs no `ro` entry (it falls back to itself, correctly), so only English-authored keys need translating."* **That is wrong, and it cost 53 strings.** It reasons about `ro` and never asks what `en` does. A Romanian-spelled key with no `en` entry is exactly the failure above — the Media source picker shipped half in Romanian on an English Mac because of it.

**Every translatable key needs an `en` entry. No exceptions.** `scripts/i18n_coverage.py` exits 1 when one is missing, in both `--check` and rewrite mode, and names the offenders. New strings: fill `en`, `ro` and `es` when you add them.

**Fixing the mixed keys is additive, not a rewrite.** The literal stays the key; the catalog supplies every language including `en`. This is safe at every intermediate state, which a mass rename of 615 literals would not be.

**Adding translations:** the catalog is plain JSON and can be hand-edited (`{"KEY": {"comment": …, "localizations": {"ro": {"stringUnit": {"state": "translated", "value": …}}}}}`); Xcode's String Catalog editor reconciles it against the extracted `.stringsdata` on the next build.

**Write it back the way Xcode does, or the next build reformats the whole file** into a 20k-line diff. Xcode uses a space before the colon (`"key" : value`), 2-space indent, no trailing newline — reproduce it exactly with `json.dumps(d, ensure_ascii=False, indent=2, separators=(',', ' : '))`. Build once after hand-editing and re-check `git diff`: Xcode also MERGES the `comment` fields of a key used from more than one call site, and that edit is its to make, not yours to fight. Verify with `ls …/TopPresenter.app/Contents/Resources | grep lproj` and `plutil -p …/ro.lproj/Localizable.strings` — an empty catalog silently produces NO `.lproj` at all, so a green build proves nothing on its own.
