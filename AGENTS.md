...
- **The floor for ~15 visible controls is ~100 ms in Debug.** Below that means fewer controls on screen at once, which is a design decision, not an optimisation. Release was not measured: `-configuration Release` cannot run the tests (`@testable` needs `-enable-testing`, and forcing `ENABLE_TESTABILITY=YES` trips `isolated deinit is marked isolated, but containing class 'PresentationManager' is not isolated to an actor`). Plain Release builds fine — the shipped app is likely materially faster than a Debug run in Xcode
